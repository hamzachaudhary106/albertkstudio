  ...
  return (
    <section id="team" className="premium-section section-divide bg-white">
      <div className="page-wrap">
        ...
        <StaggerReveal
          stagger={0.15}
          className="card-grid-equal grid md:grid-cols-3 gap-6 lg:gap-8"
        >
          {stylists.map((stylist, i) => (
            <StaggerItem
              ...
              className="h-full"
            >
              <article className="card-equal group border border-curly-border overflow-hidden">
                <div className="relative aspect-[4/5] shrink-0 overflow-hidden">
                  {stylist.image ? (
                    <SafeImage
                      src={stylist.image}
                      alt={`${stylist.name}, ${stylist.role} at Albert K Studio`}
                      className="absolute inset-0 w-full h-full object-cover object-[center_20%] transition-transform duration-700 group-hover:scale-[1.03]"
                    />
                  ) : (
